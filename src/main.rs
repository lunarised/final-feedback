...
async fn main() -> std::io::Result<()> {
    ...
    let player_datacenter = env::var("PLAYER_DATACENTER").unwrap_or_else(|_| "Datacenter".to_string());
    let banner_image = env::var("BANNER_IMAGE").unwrap_or_else(|_| "/assets/banner.webp".to_string());
    let profile_image = env::var("PROFILE_IMAGE").unwrap_or_else(|_| "/assets/profile.webp".to_string());
    let tagline = env::var("TAGLINE").unwrap_or_else(|_| "Ran content with me? Let me know how I did!".to_string());
    ...
    log::info!("Rate limit window: {} minutes", rate_limit_minutes);
